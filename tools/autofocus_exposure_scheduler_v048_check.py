...
need('CMakeLists.txt','VERSION 0.2.10.48')
...
need('src/gui/main_window.h','histogramAutoConverged_','histogramAutoBestExposure_','sessionStartMode_','sessionStartAt_')
need('src/gui/main_window.cpp','hiPct-lo<8||lo>=248','AUTO LOCKED near optimum','Histogram auto exposure LOCKED','Use current telescope pointing (J2000)',
     'Use current mount pointing as target','Update selected','Delete selected','Clear plan','Move up','Move down','Start at date/time')
need('src/core/astro_types.h','QDateTime startAtUtc','QDateTime scheduledStartUtc','startAtUtc')
need('src/algorithms/scheduler.cpp','"scheduled"','"waiting-start"','"waiting-camera"','beginScheduled')
...
need('docs/openapi.yaml','version: 0.2.10.48','startAtUtc:')
need('docs/SCHEDULER.md','v0.2.10.48','waiting-camera')
print(f'PASS v0.2.10.48 AF/exposure/scheduler lifecycle: {checks} assertions')
