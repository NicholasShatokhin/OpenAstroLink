...
def need(rel,*tokens):
    ...
    return text
need('CMakeLists.txt','VERSION 0.2.10.59')
...
gui=need('src/gui/main_window.cpp','P99.5','sparseBright','const double controlTarget=sparseBright?0.62','histogramAutoConverged_','histogramAutoLastGain_','std::sqrt(current*histogramAutoPreviousExposure_)','f->addRow("Offset",liveOffset_)','Observatory device classes — API placeholders')
ser=need('src/core/ser_writer.cpp','OpenAstroLink planetary capture metadata','ExposureRequestedSec=','ExposureActualFirstFrameSec=','GainRequested=','OffsetRequested=','MeasuredRecordFPS=','TimestampTrailerUTC=true')
...
need('docs/openapi.yaml','version: 0.2.10.59')
...
print(f'histogram/SER/auxiliary stubs current regression: PASS ({len(checks)} assertions)')
