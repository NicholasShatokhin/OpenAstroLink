...
#include <cstdint>
#define EDSCALLBACK
using EdsVoid=void; using EdsBool=int; using EdsError=std::uint32_t; using EdsUInt32=std::uint32_t; using EdsUInt64=std::uint64_t; using EdsInt32=std::int32_t; using EdsObjectEvent=std::uint32_t; using EdsStateEvent=std::uint32_t;
...
static constexpr EdsStateEvent kEdsStateEvent_Shutdown=0x301, kEdsStateEvent_All=0x300;
...
using Handler=EdsError(*)(EdsObjectEvent,EdsBaseRef,EdsVoid*); using StateHandler=EdsError(*)(EdsStateEvent,EdsUInt32,EdsVoid*); using AddedHandler=EdsError(*)(EdsVoid*);
...
inline EdsError EdsSetObjectEventHandler(EdsCameraRef,EdsObjectEvent,Handler,EdsVoid*){return 0;} inline EdsError EdsSetCameraStateEventHandler(EdsCameraRef,EdsStateEvent,StateHandler,EdsVoid*){return 0;} inline EdsError EdsSetPropertyData(EdsBaseRef,EdsUInt32,EdsInt32,EdsUInt32,const EdsVoid*){return 0;}
