...
## Пошук і ручний вибір послідовного порту (v0.2.10.10)
...
Якщо контролер перезапускається під час відкриття COM-порту, `oal.gemini` робить швидкий `:02#` probe, а після невдачі залишає той самий порт відкритим до завершення `resetRecoveryMs` (типово 2000 мс) і повторює probe. Це відповідає реальній HIL-поведінці Gemini EAF, де ручний тест без паузи не отримував відповіді, а після 2 с повертав `EOK#`.


### Serial-діагностика в `oal-hardware-probe` (v0.2.10.10)
...
## v0.2.10.10 — виправлення таймінгів у manifest
