...
bool OalWsServer::start(quint16 p,QString*e){stop();if(!server_.listen(QHostAddress::Any,p)){if(e)*e=server_.errorString();return false;}timer_.start();return true;}void OalWsServer::stop(){timer_.stop();for(auto*c:clients_){if(!c)continue;c->disconnect(this);c->close();delete c;}clients_.clear();server_.close();}
