...
SkyFrame solvedSkyFrame(const CameraFrame &frame,const SolveResult &solve){
    SkyFrame f;if(!solve.success||frame.image.empty()||solve.scaleArcsecPerPx<=0.0)return f;
    f.valid=true;f.source="solve";f.label="Solved main camera";f.center={solve.raDeg,solve.decDeg,solve.frame};
    f.widthDeg=solve.scaleArcsecPerPx*double(frame.image.cols)/3600.0;
    f.heightDeg=solve.scaleArcsecPerPx*double(frame.image.rows)/3600.0;
    f.rotationDeg=solve.rotationDeg;f.widthPx=frame.image.cols;f.heightPx=frame.image.rows;f.measured=true;return f;
}
QJsonObject solveEventJson(const SolveResult &solve,const SkyFrame &frame){
    auto j=solveToJson(solve);j["lastSolvedFrame"]=skyFrameToJson(frame);return j;
}
    ...
    const QString id=operations_.submit("solver.adaptive",{"camera","solver"},true,[this,cam,solver,profile,r](OperationContext&ctx){
        ...
        for(int attempt=0;attempt<r.maxAttempts;++attempt){
            ...
            if(lastResult.success){
                QMetaObject::invokeMethod(this,[this,frame=lastPrepared.frame,result=lastResult](){commitCapturedFrame(frame);lastSolve_=result;if(auto sf=solvedSkyFrame(frame,lastSolve_);sf.valid)lastSolvedFrame_=sf;auto j=solveEventJson(lastSolve_,lastSolvedFrame_);emit solveCompleted(j);if(oalWsServer_)oalWsServer_->broadcast("solveResult",j);emit logMessage(lastSolve_.message);emitState();},Qt::BlockingQueuedConnection);
        ...
        if(!lastPrepared.frame.image.empty())QMetaObject::invokeMethod(this,[this,frame=lastPrepared.frame,result=lastResult](){commitCapturedFrame(frame);lastSolve_=result;if(auto sf=solvedSkyFrame(frame,lastSolve_);sf.valid)lastSolvedFrame_=sf;auto j=solveEventJson(lastSolve_,lastSolvedFrame_);emit solveCompleted(j);if(oalWsServer_)oalWsServer_->broadcast("solveResult",j);emit logMessage(lastSolve_.message);emitState();},Qt::BlockingQueuedConnection);
