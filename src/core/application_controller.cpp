...
ApplicationController::ApplicationController(QObject *parent):QObject(parent),scheduler_(this){
...
#endif
else{if(err)*err="Unknown camera backend";return false;}if(!d->connectDevice(err))return false;camera_=std::move(d);emit logMessage("Camera connected: "+camera_->displayName());emitState();return true;}
...
#endif
else{if(err)*err="Unknown mount backend";return false;}if(!d->connectDevice(err))return false;mount_=std::move(d);emit logMessage("Mount connected: "+mount_->displayName());emitState();return true;}
...
#endif
else{if(err)*err="Unknown focuser backend";return false;}if(!d->connectDevice(err))return false;focuser_=std::move(d);emit logMessage("Focuser connected: "+focuser_->displayName());emitState();return true;}
...
void ApplicationController::clearPolarSamples(){polarSamples_.clear();emit polarSampleCountChanged(0);}
bool ApplicationController::addPolarSample(QString*e){if(!lastSolve_.success){if(e)*e="Last plate solve is invalid";return false;}MountStatus m;if(!mountStatus(m,e))return false;polarSamples_.push_back({lastSolve_,m.coordinate.raDeg});emit polarSampleCountChanged(int(polarSamples_.size()));return true;}
...
PolarAlignmentResult ApplicationController::estimatePolarAlignment()const{return polarEstimator_.estimate(polarSamples_,profile_.observer,QDateTime::currentDateTimeUtc());}
