...
void AscomClassicMount::updateEquatorialSystem(int value){equatorialSystem_=value;nativeFrame_=value==1?EquatorialFrame::JNow:EquatorialFrame::J2000;}
...
bool AscomClassicMount::status(MountStatus&s,QString*e){QJsonObject d;if(!request({{"cmd","status"}},&d,e))return false;updateEquatorialSystem(d.value("equatorialSystem").toInt(equatorialSystem_));EquatorialCoord native{d.value("raHours").toDouble()*15.0,d.value("decDeg").toDouble(),nativeFrame_};s.connection=state_;s.coordinate=fromAscomFrame(native);s.tracking=d.value("tracking").toBool();s.slewing=d.value("slewing").toBool();s.parked=d.value("parked").toBool();const int side=d.value("sideOfPier").toInt(-1);s.pierSide=side==0?"east":side==1?"west":"unknown";return true;}
