...
HttpJsonClient::Reply HttpJsonClient::postForm(const QUrl&u,const QUrlQuery&f,int t){QNetworkAccessManager manager;QNetworkRequest req(u);req.setHeader(QNetworkRequest::ContentTypeHeader,"application/x-www-form-urlencoded");req.setHeader(QNetworkRequest::UserAgentHeader,"OpenAstroSuite/" OAS_VERSION);req.setRawHeader("Connection","close");return wait(manager.post(req,f.toString(QUrl::FullyEncoded).toUtf8()),t);}
