...
HttpJsonClient::Reply HttpJsonClient::get(const QUrl&u,int t){QNetworkRequest req(u);req.setHeader(QNetworkRequest::UserAgentHeader,"OpenAstroSuite/0.2");req.setRawHeader("Connection","close");return wait(manager_.get(req),t);}
HttpJsonClient::Reply HttpJsonClient::postJson(const QUrl&u,const QJsonObject&b,int t){QNetworkRequest req(u);req.setHeader(QNetworkRequest::ContentTypeHeader,"application/json");req.setHeader(QNetworkRequest::UserAgentHeader,"OpenAstroSuite/0.2");req.setRawHeader("Connection","close");return wait(manager_.post(req,QJsonDocument(b).toJson(QJsonDocument::Compact)),t);}
HttpJsonClient::Reply HttpJsonClient::putForm(const QUrl&u,const QUrlQuery&f,int t){QNetworkRequest req(u);req.setHeader(QNetworkRequest::ContentTypeHeader,"application/x-www-form-urlencoded");req.setHeader(QNetworkRequest::UserAgentHeader,"OpenAstroSuite/0.2");req.setRawHeader("Connection","close");return wait(manager_.put(req,f.toString(QUrl::FullyEncoded).toUtf8()),t);}
}
