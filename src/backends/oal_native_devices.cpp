...
bool NativeOalMount::status(MountStatus &s, QString *error){QJsonObject d;if(!invokeOk("mount.status",{},&d,error))return false;s.connection=state_;s.coordinate={d.value("raDeg").toDouble(),d.value("decDeg").toDouble()};s.tracking=d.value("tracking").toBool();s.slewing=d.value("slewing").toBool();s.parked=d.value("parked").toBool();s.pierSide=d.value("pierSide").toString("unknown");return true;}
