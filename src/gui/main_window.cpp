...
QWidget *MainWindow::buildLiveFinderTab(){
    ...
    liveExposure_=dspin(0.0001,10.0,0.001,4);liveGain_=ispin(0,102400,0);liveBin_=ispin(1,4,2);liveFps_=dspin(0.2,10.0,5.0,1);
    ...
    f->addRow("Exposure, s",liveExposure_);f->addRow("Gain",liveGain_);f->addRow("Bin",liveBin_);f->addRow("Target FPS",liveFps_);f->addRow(liveAutoStretch_);f->addRow(liveCrosshair_);f->addRow(liveHighlight_);f->addRow(liveDebayer_);f->addRow("Bayer pattern",liveBayerPattern_);
    ...
    connect(liveViewButton_,&QPushButton::clicked,this,[this](){
        ...
        LiveViewRequest r;r.exposureSec=liveExposure_->value();r.gain=liveGain_->value();r.binX=r.binY=liveBin_->value();r.targetFps=liveFps_->value();r.debayer=liveDebayer_&&liveDebayer_->isChecked();if(liveBayerPattern_){const QString p=liveBayerPattern_->currentIndex()==0?"AUTO":liveBayerPattern_->currentText();r.bayerPattern=bayerPatternFromString(p);}QString e;const QString id=c_->startLiveView(r,&e);if(id.isEmpty()){showError(e);return;}liveViewOperationId_=id;setLiveViewBusy(true);appendLog(QString("Live View started: %1").arg(id));
    });
    ...
    auto*note=new QLabel("Safety: never point an unfiltered telescope/camera at the Sun. For daylight alignment use a distant terrestrial target. Daylight QHY start: about 1 ms, gain 0; if the image is white, reduce exposure before changing focus. QHY and ZWO Live View use native continuous SDK streams where available. Debayer is optional and camera-neutral: Auto uses driver CFA metadata, while RGGB/BGGR/GRBG/GBRG can be selected manually. Science FITS/RAW data is never debayered. Canon DSLR repeated-shutter Live View is intentionally blocked until the dedicated EDSDK EVF transport is added.");note->setWordWrap(true);l->addWidget(note);l->addStretch();return w;
}
...
QWidget *MainWindow::buildCaptureTab(){
    ...
    auto*histGroup=new QGroupBox("Preview histogram / exposure assistant");auto*histLayout=new QVBoxLayout(histGroup);auto*histControls=new QFormLayout;histogramEnabled_=new QCheckBox("Compute after each received frame");histogramAutoExposure_=new QCheckBox("Auto-apply suggested exposure to next capture");histogramTarget_=dspin(3.0,40.0,18.0,1);histogramTarget_->setSuffix(" %");histControls->addRow(histogramEnabled_);histControls->addRow("Target background",histogramTarget_);histControls->addRow(histogramAutoExposure_);histLayout->addLayout(histControls);histogramView_=new QLabel("Histogram disabled");histogramView_->setAlignment(Qt::AlignCenter);histogramView_->setMinimumHeight(150);histogramView_->setStyleSheet("background:#101010;color:#bbb;border:1px solid #444");histogramStats_=new QLabel("Uses the operational preview (Canon: embedded JPEG), so exposure guidance is approximate rather than RAW-linear.");histogramStats_->setWordWrap(true);histogramApplyButton_=new QPushButton("Apply suggested exposure");histogramApplyButton_->setEnabled(false);histLayout->addWidget(histogramView_);histLayout->addWidget(histogramStats_);histLayout->addWidget(histogramApplyButton_);connect(histogramEnabled_,&QCheckBox::toggled,this,[this](bool on){if(!on){histogramView_->setText("Histogram disabled");histogramView_->setPixmap(QPixmap());histogramStats_->setText("Uses the operational preview (Canon: embedded JPEG), so exposure guidance is approximate rather than RAW-linear.");histogramApplyButton_->setEnabled(false);histogramSuggestedExposure_=0.0;}else if(!lastImage_.isNull())updateHistogram(lastImage_);});connect(histogramApplyButton_,&QPushButton::clicked,this,[this](){if(histogramSuggestedExposure_>0.0){exposure_->setValue(histogramSuggestedExposure_);appendLog(QString("Histogram suggestion applied: next exposure %1 s; ISO/gain unchanged").arg(histogramSuggestedExposure_,0,'g',5));}});l->addWidget(histGroup);
...
void MainWindow::updateHistogram(const QImage &image){
    ...
    const double median=double(p50)/255.0,target=histogramTarget_?histogramTarget_->value()/100.0:0.18;double factor=target/std::max(0.01,median);factor=std::clamp(factor,0.5,2.0);if(highClip>0.005)factor=std::min(factor,0.5);if(lowClip>0.65&&median<0.03)factor=std::max(factor,1.5);
    const double current=std::max(0.0001,exposure_?exposure_->value():1.0);histogramSuggestedExposure_=std::clamp(current*factor,0.0001,3600.0);const double delta=std::abs(histogramSuggestedExposure_/current-1.0);
    QString advice;if(highClip>0.005)advice="highlights clipped — shorten exposure";else if(median<target*0.70)advice="background low — lengthen exposure";else if(median>target*1.35)advice="background high — shorten exposure";else advice="background near target";
    histogramStats_->setText(QString("Preview histogram (log scale): P1=%1%, median=%2%, P99=%3%, low clip=%4%, high clip=%5%. %6. Suggested next exposure: %7 s (ISO/gain unchanged).")
        .arg(100.0*p01/255.0,0,'f',1).arg(100.0*p50/255.0,0,'f',1).arg(100.0*p99/255.0,0,'f',1).arg(100.0*lowClip,0,'f',2).arg(100.0*highClip,0,'f',2).arg(advice).arg(histogramSuggestedExposure_,0,'g',5));
    histogramApplyButton_->setEnabled(delta>0.03);if(histogramAutoExposure_&&histogramAutoExposure_->isChecked()&&delta>0.03){exposure_->setValue(histogramSuggestedExposure_);appendLog(QString("Histogram auto exposure: next capture %1 s (preview-derived; ISO/gain unchanged)").arg(histogramSuggestedExposure_,0,'g',5));}
}
...
QWidget *MainWindow::buildMountTab(){
    ...
    auto*syncWarning=new QLabel("SYNC SAFETY: Sync means the telescope is PHYSICALLY pointing at the entered target now. Mechanical Park alone is not a sky-coordinate Sync. For your intended workflow: park/home near Polaris → center Polaris (or plate-solve) → Sync once → then Stellarium clicks and coordinate GOTO use that sky anchor.");syncWarning->setWordWrap(true);syncWarning->setStyleSheet("color:#a40;font-weight:bold");l->addWidget(syncWarning);
    ...
    auto*orientationBox=new QGroupBox("Native mount orientation calibration");auto*orientationLayout=new QVBoxLayout(orientationBox);auto*orientationInfo=new QLabel("After a CORRECT Sync, test only small sky offsets first. If RA/tracking moves opposite, reverse Axis 1. If DEC moves opposite, reverse Axis 2. Every sign change invalidates Sync.");orientationInfo->setWordWrap(true);orientationLayout->addWidget(orientationInfo);auto*reverseA1=new QPushButton("Reverse Axis 1 mapping / tracking direction");auto*reverseA2=new QPushButton("Reverse Axis 2 mapping");connect(reverseA1,&QPushButton::clicked,this,[this](){auto p=c_->profile();p.mount.axis1Sign=p.mount.axis1Sign>=0?-1:1;c_->setProfile(p);appendLog(QString("Axis 1 mapping reversed to %1; perform Sync again before tracking/GOTO").arg(p.mount.axis1Sign));});connect(reverseA2,&QPushButton::clicked,this,[this](){auto p=c_->profile();p.mount.axis2Sign=p.mount.axis2Sign>=0?-1:1;c_->setProfile(p);appendLog(QString("Axis 2 mapping reversed to %1; perform Sync again before GOTO").arg(p.mount.axis2Sign));});orientationLayout->addWidget(reverseA1);orientationLayout->addWidget(reverseA2);l->addWidget(orientationBox);
    auto*setPark=new QPushButton("Set current mechanical axes as Park");connect(setPark,&QPushButton::clicked,this,[this](){MountStatus st;QString e;if(!c_->mountStatus(st,&e)){showError(e);return;}if(!st.axes.valid){showError("This backend does not expose mechanical axis coordinates");return;}auto p=c_->profile();p.mount.parkAxis1Deg=st.axes.axis1Deg;p.mount.parkAxis2Deg=st.axes.axis2Deg;p.mount.customPark=true;c_->setProfile(p);appendLog(QString("Mechanical Park saved: axis1=%1°, axis2=%2°. Park is mechanical; perform a sky Sync after startup before GOTO.").arg(st.axes.axis1Deg,0,'f',4).arg(st.axes.axis2Deg,0,'f',4));});auto*defaultPark=new QPushButton("Clear mechanical Park calibration (disable Park)");connect(defaultPark,&QPushButton::clicked,this,[this](){auto p=c_->profile();p.mount.customPark=false;c_->setProfile(p);appendLog("Mechanical Park calibration cleared; Park is disabled until current safe axes are saved again");});l->addWidget(setPark);l->addWidget(defaultPark);
...
void MainWindow::synchronizeMountCoordinatesFrom(const QString &system){
    ...
    mountCoordinateSyncing_=true;const QDateTime utc=QDateTime::currentDateTimeUtc();const auto observer=c_->profile().observer;EquatorialCoord j2000;
