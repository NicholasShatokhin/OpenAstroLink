...
QWidget *MainWindow::buildSkyMapPanel(){
    ...
    skyMap_=new SkyMapWidget;skyMap_->setToolTip("Offline horizon map: drag to pan, wheel to zoom, click to select, double-click to slew");l->addWidget(skyMap_,1);
    skySelection_=new QLabel("Select an object on the map or search by name.");skySelection_->setWordWrap(true);skySelection_->setMinimumHeight(45);l->addWidget(skySelection_);
    ...
    auto findObject=[this](){if(!skyMap_->selectObjectByName(skySearch_->text())){showError("Sky Map object not found: "+skySearch_->text());return;}skyMap_->focusSelected();updateSkySelectionText();};
    ...
    connect(skyMap_,&SkyMapWidget::selectionChanged,this,[this](const QString&,double,double){updateSkySelectionText();});
    auto slewSelected=[this](){if(!skyMap_||!skyMap_->hasSelection()){showError("Select a Sky Map target first");return;}QString e;const auto target=skyMap_->selectedCoordinate();if(!c_->slewMount(target,&e)){showError(e.isEmpty()?"Mount GOTO rejected":e);return;}appendLog(QString("Sky Map GOTO: %1 — RA=%2° DEC=%3° J2000").arg(skyMap_->selectedName()).arg(target.raDeg,0,'f',6).arg(target.decDeg,0,'f',6));};
    ...
    connect(sync,&QPushButton::clicked,this,[this](){if(!skyMap_||!skyMap_->hasSelection()){showError("Select a Sky Map target first");return;}QString e;const auto target=skyMap_->selectedCoordinate();if(!c_->syncMount(target,&e))showError(e.isEmpty()?"Mount Sync rejected":e);else appendLog("Sky Map Sync: "+skyMap_->selectedName());});
    ...
    connect(schedule,&QPushButton::clicked,this,[this](){if(!skyMap_||!skyMap_->hasSelection()){showError("Select a Sky Map target first");return;}if(!targetRa_||!targetDec_||!targetName_){showError("Scheduler controls are not ready yet");return;}const auto target=skyMap_->selectedCoordinate();targetName_->setText(skyMap_->selectedName());targetRa_->setValue(target.raDeg);targetDec_->setValue(target.decDeg);if(tabs_)for(int i=0;i<tabs_->count();++i)if(tabs_->tabText(i)=="Scheduler"){tabs_->setCurrentIndex(i);break;}appendLog("Scheduler target set from Sky Map: "+skyMap_->selectedName());});
    skyMapTimer_=new QTimer(this);skyMapTimer_->setInterval(5000);connect(skyMapTimer_,&QTimer::timeout,this,[this](){if(!skyMap_)return;skyMap_->setUtc(QDateTime::currentDateTimeUtc());updateSkySelectionText();});skyMapTimer_->start();
    return w;
}

void MainWindow::refreshSkyMapContext(){
    if(!skyMap_)return;const auto p=c_->profile();skyMap_->setObserver(p.observer);skyMap_->setUtc(QDateTime::currentDateTimeUtc());const double scale=p.arcsecPerPixel();skyMap_->setFovDegrees(scale>0.0?scale*p.sensorWidthPx/3600.0:0.0,scale>0.0?scale*p.sensorHeightPx/3600.0:0.0);const auto &solve=c_->lastSolve();skyMap_->setSolvedCoordinate({solve.raDeg,solve.decDeg,solve.frame},solve.success);MountStatus st;QString e;if(c_->mountStatus(st,&e)&&st.coordinateValid)skyMap_->setMountCoordinate(st.coordinate,true);else skyMap_->setMountCoordinate({},false);updateSkySelectionText();
}
...
void MainWindow::updateSkySelectionText(){
    if(!skySelection_||!skyMap_)return;if(!skyMap_->hasSelection()){skySelection_->setText("Select an object on the map or search by name. Double-clicking an object sends a GOTO through the active OAL mount backend.");return;}const auto eq=skyMap_->selectedCoordinate();const auto h=skyMap_->selectedHorizontal();const double raHours=eq.raDeg/15.0;skySelection_->setText(QString("%1  [%2]   RA %3 h / %4°   DEC %5°   Alt %6°   Az %7°   mag %8%9").arg(skyMap_->selectedName(),skyMap_->selectedKind()).arg(raHours,0,'f',4).arg(eq.raDeg,0,'f',5).arg(eq.decDeg,0,'f',5).arg(h.altDeg,0,'f',2).arg(h.azDeg,0,'f',2).arg(skyMap_->selectedMagnitude(),0,'f',2).arg(h.altDeg<0.0?"   — below horizon":""));
}
