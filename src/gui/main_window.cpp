...
static SkyFrame profileSkyFrame(const QString &source,const QString &label,const EquatorialCoord &center,double scaleArcsecPerPx,int widthPx,int heightPx,double rotationDeg){
    SkyFrame f;if(scaleArcsecPerPx<=0.0||widthPx<=0||heightPx<=0)return f;f.valid=true;f.source=source;f.label=label;f.center=convertEquatorialFrame(center,EquatorialFrame::J2000);f.widthDeg=scaleArcsecPerPx*double(widthPx)/3600.0;f.heightDeg=scaleArcsecPerPx*double(heightPx)/3600.0;f.rotationDeg=rotationDeg;f.widthPx=widthPx;f.heightPx=heightPx;f.measured=false;return f;
}
...
static bool monochromeLike(const QImage &input){
    if(input.isNull())return false;
    const QImage rgb=input.convertToFormat(QImage::Format_RGB888);
    const qsizetype pixels=qsizetype(rgb.width())*rgb.height();
    const int step=std::max(1,int(std::sqrt(double(std::max<qsizetype>(1,pixels))/20000.0)));
    qsizetype tested=0,nonMono=0;
    for(int y=0;y<rgb.height();y+=step){const uchar*r=rgb.constScanLine(y);for(int x=0;x<rgb.width();x+=step){const int rr=r[3*x],gg=r[3*x+1],bb=r[3*x+2];if(std::max({rr,gg,bb})-std::min({rr,gg,bb})>4)++nonMono;++tested;}}
    return tested>0&&double(nonMono)/double(tested)<0.01;
}
static QImage blackToRedPreview(const QImage &input){
    if(input.isNull())return input;
    const QImage gray=input.convertToFormat(QImage::Format_Grayscale8);QImage out(gray.width(),gray.height(),QImage::Format_RGB888);
    for(int y=0;y<gray.height();++y){const uchar*g=gray.constScanLine(y);uchar*r=out.scanLine(y);for(int x=0;x<gray.width();++x){r[3*x]=g[x];r[3*x+1]=0;r[3*x+2]=0;}}
    return out;
}
...
MainWindow::MainWindow(ObservatoryController*c,QWidget*p):QMainWindow(p),c_(c){uiTheme_=loadUiTheme();setWindowTitle("OpenAstroSuite / OpenAstroLink");resize(1500,900);statusBar()->showMessage(QString("Core: %1 — %2").arg(c_->controlMode(),c_->endpointDescription()));auto*central=new QWidget;auto*root=new QHBoxLayout(central);auto*split=new QSplitter(Qt::Horizontal);root->addWidget(split);setCentralWidget(central);
    auto*left=new QWidget;auto*ll=new QVBoxLayout(left);leftTabs_=new QTabWidget;
    auto*imagingPage=new QWidget;auto*imagingLayout=new QVBoxLayout(imagingPage);auto*images=new QSplitter(Qt::Horizontal);rawImage_=new QLabel("No camera frame");astroImage_=new QLabel("No astrometry overlay");for(auto*l:{rawImage_,astroImage_}){l->setAlignment(Qt::AlignCenter);l->setMinimumSize(420,300);l->setObjectName("oasPreviewSurface");}images->addWidget(rawImage_);images->addWidget(astroImage_);imagingLayout->addWidget(images,3);starScene_=new QGraphicsScene(this);starMapView_=new QGraphicsView(starScene_);starMapView_->setObjectName("oasStarMap");starMapView_->setMinimumHeight(220);imagingLayout->addWidget(starMapView_,1);leftTabs_->addTab(imagingPage,"Imaging");
    auto*dualPage=new QWidget;auto*dualLayout=new QVBoxLayout(dualPage);auto*dualImages=new QSplitter(Qt::Horizontal);dualMainImage_=new QLabel("Main camera — no live stream");dualGuideImage_=new QLabel("Guide camera — no live stream");for(auto*l:{dualMainImage_,dualGuideImage_}){l->setAlignment(Qt::AlignCenter);l->setMinimumSize(360,300);l->setObjectName("oasPreviewSurface");}dualImages->addWidget(dualMainImage_);dualImages->addWidget(dualGuideImage_);dualLayout->addWidget(dualImages,1);dualLiveStats_=new QLabel("Dual Live idle");dualLiveStats_->setWordWrap(true);dualLayout->addWidget(dualLiveStats_);leftTabs_->addTab(dualPage,"Dual Live");
    leftTabs_->addTab(buildSkyMapPanel(),"Sky Map");ll->addWidget(leftTabs_,1);log_=new QTextEdit;log_->setReadOnly(true);log_->setMaximumHeight(170);ll->addWidget(log_);split->addWidget(left);
    ...
    auto*viewMenu=menuBar()->addMenu("&View");auto*themeMenu=viewMenu->addMenu("Night vision mode");auto*themeGroup=new QActionGroup(this);themeGroup->setExclusive(true);
    auto addThemeAction=[this,themeMenu,themeGroup](const QString &name,UiTheme theme){auto*a=themeMenu->addAction(name);a->setCheckable(true);a->setData(int(theme));a->setChecked(theme==uiTheme_);themeGroup->addAction(a);};
    addThemeAction("Normal",UiTheme::Normal);addThemeAction("Night Vision",UiTheme::NightVision);addThemeAction("Strict Night Mode",UiTheme::StrictNight);
    connect(themeGroup,&QActionGroup::triggered,this,[this](QAction*a){applyTheme(UiTheme(a->data().toInt()));});
    auto*cycleTheme=viewMenu->addAction("Cycle night vision mode");cycleTheme->setShortcut(QKeySequence("Ctrl+Shift+N"));connect(cycleTheme,&QAction::triggered,this,[this,themeGroup](){const UiTheme next=uiTheme_==UiTheme::Normal?UiTheme::NightVision:(uiTheme_==UiTheme::NightVision?UiTheme::StrictNight:UiTheme::Normal);applyTheme(next);for(auto*a:themeGroup->actions())a->setChecked(a->data().toInt()==int(next));});
    connect(c_,&ObservatoryController::logMessage,this,&MainWindow::appendLog);connect(c_,&ObservatoryController::videoFrameCaptured,this,[this](const QImage&i,const QString&role,const QJsonObject&stats){QLabel*target=role=="guide"?dualGuideImage_:dualMainImage_;if(target&&!i.isNull()){const bool debayered=role=="main"&&liveDebayer_&&liveDebayer_->isChecked();QImage shown=displayPreviewImage(i,debayered);QPainter p(&shown);p.setPen(QPen(themeColor(QColor(80,255,120)),2));const QPointF c(shown.width()/2.0,shown.height()/2.0);const int arm=std::max(24,std::min(shown.width(),shown.height())/12);p.drawLine(QPointF(c.x()-arm,c.y()),QPointF(c.x()+arm,c.y()));p.drawLine(QPointF(c.x(),c.y()-arm),QPointF(c.x(),c.y()+arm));p.end();target->setPixmap(QPixmap::fromImage(shown).scaled(target->size(),Qt::KeepAspectRatio,Qt::FastTransformation));}if(role=="guide")lastGuideLiveStats_=stats;else lastMainLiveStats_=stats;if(dualLiveStats_){auto row=[](const QString&name,const QJsonObject&o){if(o.isEmpty())return name+": idle";QString text=QString("%1: capture %2 fps | preview %3 fps | record %4 fps | preview drop %5 | record drop %6 | %7-bit").arg(name).arg(o.value("captureFps").toDouble(),0,'f',1).arg(o.value("previewFps").toDouble(),0,'f',1).arg(o.value("recordFps").toDouble(),0,'f',1).arg(o.value("previewDropped").toVariant().toLongLong()).arg(o.value("recordDropped").toVariant().toLongLong()).arg(o.value("bitsPerSample").toInt(8));const QString transport=o.value("networkTransport").toString();if(!transport.isEmpty())text+=" | "+transport;return text;};dualLiveStats_->setText(row("Main",lastMainLiveStats_)+"\n"+row("Guide",lastGuideLiveStats_));}});connect(c_,&ObservatoryController::frameCaptured,this,[this](const QImage&i,const QString&id){lastImage_=i;renderCameraFrame(i);const bool scienceFrame=!id.startsWith("live-")&&!id.startsWith("af-preview-");if(histogramEnabled_&&histogramEnabled_->isChecked())updateHistogram(i,scienceFrame);if(scienceFrame)appendLog("Frame: "+id);if(!pendingSolveFrameId_.isEmpty()&&pendingSolveFrameId_==id){pendingSolveFrameId_.clear();QTimer::singleShot(0,this,[this](){SolveHint h;h.raDeg=hintRa_->value();h.decDeg=hintDec_->value();h.searchRadiusDeg=hintRadius_->value();c_->solveLast(h);});}});connect(c_,&ObservatoryController::solveCompleted,this,[this](const QJsonObject&o){appendLog(QString("Solve: %1 RA=%2 DEC=%3 matches=%4").arg(o.value("success").toBool()?"OK":"FAIL").arg(o.value("raDeg").toDouble()).arg(o.value("decDeg").toDouble()).arg(o.value("matchedStars").toInt()));updateAstrometryOverlay();updateStarMap();refreshSkyMapContext();});connect(c_,&ObservatoryController::autofocusProgress,this,[this](const QJsonObject&o){appendLog(QString("AF pos=%1 score=%2 ±%3 stars=%4").arg(o.value("position").toInt()).arg(o.value("score").toDouble()).arg(o.value("spread").toDouble()).arg(o.value("detectedStars").toInt()));if(focuserStatus_)focuserStatus_->setText(QString("Autofocus — position %1, score %2 ±%3").arg(o.value("position").toInt()).arg(o.value("score").toDouble()).arg(o.value("spread").toDouble()));});connect(c_,&ObservatoryController::autofocusCompleted,this,[this](const QJsonObject&o){appendLog(QString("AF %1 best=%2 score=%3 — %4").arg(o.value("success").toBool()?"OK":"FAIL").arg(o.value("bestPosition").toInt()).arg(o.value("bestScore").toDouble()).arg(o.value("message").toString()));if(o.value("success").toBool()&&focusPosition_){QSignalBlocker b(focusPosition_);focusPosition_->setValue(o.value("bestPosition").toInt());focusTargetInitialized_=true;focusTargetDirty_=false;}setAutofocusBusy(false);autofocusOperationId_.clear();refreshFocuserStatus();});connect(c_,&ObservatoryController::polarSampleCountChanged,this,[this](int n){polarSamples_->setText(QString("Samples: %1").arg(n));});connect(c_,&ObservatoryController::polarAlignmentCompleted,this,[this](const QJsonObject&o){polarResult_->setText(QString("%1\nAxis RA %2°, DEC %3°\nTotal %4′\nAdjust altitude %5′, azimuth %6′").arg(o.value("message").toString()).arg(o.value("axisRaDeg").toDouble()).arg(o.value("axisDecDeg").toDouble()).arg(o.value("totalErrorArcmin").toDouble()).arg(o.value("altitudeAdjustmentArcmin").toDouble()).arg(o.value("azimuthAdjustmentArcmin").toDouble()));appendLog(QString("Automatic Polar Alignment completed: total error %1 arcmin").arg(o.value("totalErrorArcmin").toDouble(),0,'f',2));});connect(c_,&ObservatoryController::sessionChanged,this,[this](const QJsonObject&o){appendLog(QString("Session %1: %2").arg(o.value("name").toString(),o.value("state").toString()));});connect(c_,&ObservatoryController::motionEstimated,this,[this](const QJsonObject&o){appendLog(QString("Motion valid=%1 dx=%2 px dy=%3 px rot=%4° inliers=%5").arg(o.value("valid").toBool()).arg(o.value("dxPx").toDouble()).arg(o.value("dyPx").toDouble()).arg(o.value("rotationDeg").toDouble()).arg(o.value("inliers").toInt()));});connect(c_,&ObservatoryController::stateChanged,this,&MainWindow::updateMountStatusFromState);connect(c_,&ObservatoryController::stateChanged,this,&MainWindow::updateFocuserStatusFromState);connect(c_,&ObservatoryController::stateChanged,this,&MainWindow::updateDeviceStatusFromState);connect(c_,&ObservatoryController::stateChanged,this,&MainWindow::updateSkyMapFromState);connect(c_,&ObservatoryController::operationChanged,this,&MainWindow::updateOperation);connect(c_,&ObservatoryController::profileChanged,this,[this](){auto p=c_->profile();if(lat_){lat_->setValue(p.observer.latitudeDeg);lon_->setValue(p.observer.longitudeDeg);elevation_->setValue(p.observer.elevationM);}if(profileMountGeometry_){const int i=profileMountGeometry_->findData(int(p.mount.type));if(i>=0)profileMountGeometry_->setCurrentIndex(i);}if(profileAxis1Sign_)profileAxis1Sign_->setCurrentIndex(p.mount.axis1Sign>=0?0:1);if(profileAxis2Sign_)profileAxis2Sign_->setCurrentIndex(p.mount.axis2Sign>=0?0:1);if(profilePierSide_){const int i=profilePierSide_->findText(p.mount.preferredPierSide);if(i>=0)profilePierSide_->setCurrentIndex(i);}if(homeAxis1_)homeAxis1_->setValue(p.mount.homeAxis1Deg);if(homeAxis2_)homeAxis2_->setValue(p.mount.homeAxis2Deg);if(parkAxis1_)parkAxis1_->setValue(p.mount.parkAxis1Deg);if(parkAxis2_)parkAxis2_->setValue(p.mount.parkAxis2Deg);if(autoPierFlip_)autoPierFlip_->setChecked(p.mount.allowAutomaticPierFlip);if(maxGotoAxisDelta_)maxGotoAxisDelta_->setValue(p.mount.maxGotoAxisDeltaDeg);if(mountInvertAxis1_){QSignalBlocker b(mountInvertAxis1_);mountInvertAxis1_->setChecked(p.mount.axis1Sign<0);}if(mountInvertAxis2_){QSignalBlocker b(mountInvertAxis2_);mountInvertAxis2_->setChecked(p.mount.axis2Sign<0);}if(mountSiteLat_){mountSiteLat_->setValue(p.observer.latitudeDeg);mountSiteLon_->setValue(p.observer.longitudeDeg);mountSiteElevation_->setValue(p.observer.elevationM);}if(mountPreferBackendSite_){QSignalBlocker b(mountPreferBackendSite_);mountPreferBackendSite_->setChecked(p.mount.preferBackendSite);}if(polarSafeEnabled_)polarSafeEnabled_->setChecked(p.polarMotionLimits.enabled);if(polarMinAz_)polarMinAz_->setValue(p.polarMotionLimits.minAzDeg);if(polarMaxAz_)polarMaxAz_->setValue(p.polarMotionLimits.maxAzDeg);if(polarMinAlt_)polarMinAlt_->setValue(p.polarMotionLimits.minAltDeg);if(polarMaxAlt_)polarMaxAlt_->setValue(p.polarMotionLimits.maxAltDeg);if(mountRa_)synchronizeMountCoordinatesFrom("j2000");refreshSkyMapContext();});applyTheme(uiTheme_,false);QTimer::singleShot(0,this,[this](){c_->refreshState();refreshMountStatus();refreshFocuserStatus();refreshSkyMapContext();for(const auto&v:c_->operations(false))updateOperation(v.toObject());});}

QWidget *MainWindow::buildSkyMapPanel(){
    auto*w=new QWidget;auto*l=new QVBoxLayout(w);
    auto*top=new QHBoxLayout;skySearch_=new QLineEdit;skySearch_->setPlaceholderText("Find: Vega, Polaris, M31, M42 …");auto*find=new QPushButton("Find");auto*reset=new QPushButton("Reset view");top->addWidget(skySearch_,1);top->addWidget(find);top->addWidget(reset);l->addLayout(top);
    auto*options=new QHBoxLayout;skyLabels_=new QCheckBox("Labels");skyDsos_=new QCheckBox("DSO");skyConstellations_=new QCheckBox("Constellations");skyLabels_->setChecked(true);skyDsos_->setChecked(true);skyConstellations_->setChecked(true);options->addWidget(skyLabels_);options->addWidget(skyDsos_);options->addWidget(skyConstellations_);options->addStretch();l->addLayout(options);

    auto*frames=new QGroupBox("Camera footprints / framing");auto*ff=new QGridLayout(frames);
    skyShowSolvedFrame_=new QCheckBox("Solved");skyShowSolvedFrame_->setChecked(true);
    skyShowMainFrame_=new QCheckBox("Main planned");skyShowMainFrame_->setChecked(true);
    skyShowGuideFrame_=new QCheckBox("Guide planned");skyShowGuideFrame_->setChecked(false);
    skyFrameLabels_=new QCheckBox("Frame labels");skyFrameLabels_->setChecked(true);
    skyPlannerGrid_=new QCheckBox("Mosaic planner grid");skyPlannerGrid_->setChecked(false);
    skyMainRotation_=dspin(-180.0,180.0,0.0,1);skyGuideRotation_=dspin(-180.0,180.0,0.0,1);
    ff->addWidget(skyShowSolvedFrame_,0,0);ff->addWidget(skyShowMainFrame_,0,1);ff->addWidget(skyShowGuideFrame_,0,2);ff->addWidget(skyFrameLabels_,0,3);
    ff->addWidget(new QLabel("Main PA, deg"),1,0);ff->addWidget(skyMainRotation_,1,1);ff->addWidget(new QLabel("Guide PA, deg"),1,2);ff->addWidget(skyGuideRotation_,1,3);ff->addWidget(skyPlannerGrid_,2,0,1,4);
    skyFrameInfo_=new QLabel("Frames: waiting for profile/solve context");skyFrameInfo_->setWordWrap(true);ff->addWidget(skyFrameInfo_,3,0,1,4);l->addWidget(frames);

    skyMap_=new SkyMapWidget;skyMap_->setToolTip("Offline horizon map: drag to pan, wheel to zoom, click any visible sky position to select, double-click to slew");l->addWidget(skyMap_,1);
    skySelection_=new QLabel("Select an object or click any visible sky position.");skySelection_->setWordWrap(true);skySelection_->setMinimumHeight(45);l->addWidget(skySelection_);
    auto*actions=new QGridLayout;auto*slew=new QPushButton("Slew");auto*sync=new QPushButton("Sync");auto*abort=new QPushButton("Abort slew");auto*park=new QPushButton("Park");auto*unpark=new QPushButton("Unpark");auto*schedule=new QPushButton("Use in Scheduler");actions->addWidget(slew,0,0);actions->addWidget(sync,0,1);actions->addWidget(abort,0,2);actions->addWidget(park,1,0);actions->addWidget(unpark,1,1);actions->addWidget(schedule,1,2);l->addLayout(actions);

    auto*stellarium=new QGroupBox("Stellarium frame export (Remote Control plugin)");auto*sr=new QGridLayout(stellarium);
    stellariumRemoteUrl_=new QLineEdit(QSettings("OpenAstroLink","OpenAstroSuite").value("stellarium/remoteControlUrl","http://127.0.0.1:8090").toString());
    stellariumRemoteUrl_->setPlaceholderText("http://127.0.0.1:8090");
    skyStellariumFrameSource_=new QComboBox;skyStellariumFrameSource_->addItems(QStringList{"Solved frame","Main planned frame","Guide planned frame","Mosaic envelope"});
    auto*sendStellarium=new QPushButton("Send frame");auto*hideStellarium=new QPushButton("Hide frame");
    sr->addWidget(new QLabel("Remote Control URL"),0,0);sr->addWidget(stellariumRemoteUrl_,0,1,1,3);sr->addWidget(new QLabel("Frame"),1,0);sr->addWidget(skyStellariumFrameSource_,1,1);sr->addWidget(sendStellarium,1,2);sr->addWidget(hideStellarium,1,3);l->addWidget(stellarium);

    auto findObject=[this](){if(!skyMap_->selectObjectByName(skySearch_->text())){showError("Sky Map object not found: "+skySearch_->text());return;}skyMap_->focusSelected();updateSkySelectionText();refreshSkyMapContext();};
    connect(find,&QPushButton::clicked,this,findObject);connect(skySearch_,&QLineEdit::returnPressed,this,findObject);connect(reset,&QPushButton::clicked,skyMap_,&SkyMapWidget::resetView);
    connect(skyLabels_,&QCheckBox::toggled,skyMap_,&SkyMapWidget::setShowLabels);connect(skyDsos_,&QCheckBox::toggled,skyMap_,&SkyMapWidget::setShowDsos);connect(skyConstellations_,&QCheckBox::toggled,skyMap_,&SkyMapWidget::setShowConstellations);
    connect(skyShowSolvedFrame_,&QCheckBox::toggled,skyMap_,&SkyMapWidget::setShowSolvedFrame);connect(skyShowMainFrame_,&QCheckBox::toggled,skyMap_,&SkyMapWidget::setShowMainFrame);connect(skyShowGuideFrame_,&QCheckBox::toggled,skyMap_,&SkyMapWidget::setShowGuideFrame);connect(skyFrameLabels_,&QCheckBox::toggled,skyMap_,&SkyMapWidget::setShowFrameLabels);
    connect(skyPlannerGrid_,&QCheckBox::toggled,this,[this](){refreshSkyMapContext();});connect(skyMainRotation_,qOverload<double>(&QDoubleSpinBox::valueChanged),this,[this](double v){if(mosaicRotation_&&std::abs(mosaicRotation_->value()-v)>1e-9){QSignalBlocker b(mosaicRotation_);mosaicRotation_->setValue(v);}refreshSkyMapContext();});connect(skyGuideRotation_,qOverload<double>(&QDoubleSpinBox::valueChanged),this,[this](){refreshSkyMapContext();});
    connect(skyMap_,&SkyMapWidget::selectionChanged,this,[this](const QString&,double,double){updateSkySelectionText();if(skyMap_&&skyMap_->hasSelection()&&mountRa_&&mountDec_){const auto target=skyMap_->selectedCoordinate();mountCoordinateSyncing_=true;{QSignalBlocker a(mountRa_),b(mountDec_);mountRa_->setValue(target.raDeg);mountDec_->setValue(target.decDeg);}mountCoordinateSyncing_=false;synchronizeMountCoordinatesFrom("j2000");appendLog(QString("Mount target fields copied from Sky Map: %1 — RA=%2° DEC=%3° J2000").arg(skyMap_->selectedName()).arg(target.raDeg,0,'f',6).arg(target.decDeg,0,'f',6));}refreshSkyMapContext();});
    auto slewSelected=[this](){if(!skyMap_||!skyMap_->hasSelection()){showError("Select a Sky Map target or click a sky position first");return;}QString e;const auto target=skyMap_->selectedCoordinate();if(!c_->slewMount(target,&e)){showError(e.isEmpty()?"Mount GOTO rejected":e);return;}appendLog(QString("Sky Map GOTO: %1 — RA=%2° DEC=%3° J2000").arg(skyMap_->selectedName()).arg(target.raDeg,0,'f',6).arg(target.decDeg,0,'f',6));};
    connect(slew,&QPushButton::clicked,this,slewSelected);connect(skyMap_,&SkyMapWidget::objectActivated,this,[slewSelected](const QString&,double,double){slewSelected();});
    connect(sync,&QPushButton::clicked,this,[this](){if(!skyMap_||!skyMap_->hasSelection()){showError("Select a Sky Map target or click a sky position first");return;}QString e;const auto target=skyMap_->selectedCoordinate();if(!c_->syncMount(target,&e))showError(e.isEmpty()?"Mount Sync rejected":e);else appendLog("Sky Map Sync: "+skyMap_->selectedName());});
    connect(abort,&QPushButton::clicked,this,[this](){QString e;if(!c_->abortMountMotion(&e))showError(e.isEmpty()?"Mount abort failed":e);else appendLog("Sky Map: mount abort requested");});
    connect(park,&QPushButton::clicked,this,[this](){QString e;if(!c_->parkMount(true,&e))showError(e.isEmpty()?"Park failed":e);else appendLog("Sky Map: park requested");});
    connect(unpark,&QPushButton::clicked,this,[this](){QString e;if(!c_->parkMount(false,&e))showError(e.isEmpty()?"Unpark failed":e);else appendLog("Sky Map: unpark requested");});
    connect(schedule,&QPushButton::clicked,this,[this](){if(!skyMap_||!skyMap_->hasSelection()){showError("Select a Sky Map target or click a sky position first");return;}if(!targetRa_||!targetDec_||!targetName_){showError("Scheduler controls are not ready yet");return;}const auto target=skyMap_->selectedCoordinate();targetName_->setText(skyMap_->selectedName());targetRa_->setValue(target.raDeg);targetDec_->setValue(target.decDeg);if(mosaicRotation_)mosaicRotation_->setValue(skyMainRotation_?skyMainRotation_->value():0.0);if(tabs_)for(int i=0;i<tabs_->count();++i)if(tabs_->tabText(i)=="Scheduler"){tabs_->setCurrentIndex(i);break;}appendLog("Scheduler target/framing set from Sky Map: "+skyMap_->selectedName());});
    connect(stellariumRemoteUrl_,&QLineEdit::editingFinished,this,[this](){QSettings("OpenAstroLink","OpenAstroSuite").setValue("stellarium/remoteControlUrl",stellariumRemoteUrl_->text().trimmed());});
    connect(sendStellarium,&QPushButton::clicked,this,[this](){SkyFrame frame;if(!skyMap_)return;switch(skyStellariumFrameSource_->currentIndex()){case 0:frame=skyMap_->solvedFrame();break;case 1:frame=skyMap_->mainPlannedFrame();break;case 2:frame=skyMap_->guidePlannedFrame();break;default:frame=skyMap_->plannerEnvelopeFrame();break;}QString e;StellariumRemoteControlClient rc;if(!rc.sendFrame(QUrl(stellariumRemoteUrl_->text().trimmed()),frame,&e)){showError("Stellarium frame export failed: "+e);return;}appendLog(QString("Stellarium frame exported: %1 %2° x %3° PA %4°").arg(frame.label).arg(frame.widthDeg,0,'f',3).arg(frame.heightDeg,0,'f',3).arg(frame.rotationDeg,0,'f',1));});
    connect(hideStellarium,&QPushButton::clicked,this,[this](){QString e;StellariumRemoteControlClient rc;if(!rc.hideFrame(QUrl(stellariumRemoteUrl_->text().trimmed()),&e))showError("Could not hide Stellarium frame: "+e);else appendLog("Stellarium FOV frame hidden");});
    skyMapTimer_=new QTimer(this);skyMapTimer_->setInterval(5000);connect(skyMapTimer_,&QTimer::timeout,this,[this](){if(!skyMap_)return;skyMap_->setUtc(QDateTime::currentDateTimeUtc());updateSkySelectionText();refreshSkyMapContext();});skyMapTimer_->start();
    return w;
}

void MainWindow::refreshSkyMapContext(){
    if(!skyMap_)return;const auto p=c_->profile();skyMap_->setObserver(p.observer);skyMap_->setUtc(QDateTime::currentDateTimeUtc());
    MountStatus st;QString e;const bool mountValid=c_->mountStatus(st,&e)&&st.coordinateValid;if(mountValid)skyMap_->setMountCoordinate(st.coordinate,true);else skyMap_->setMountCoordinate({},false);
    const auto solved=c_->lastSolvedFrame();skyMap_->setSolvedFrame(solved);if(solved.valid)skyMap_->setSolvedCoordinate(solved.center,true);else{const auto &solve=c_->lastSolve();skyMap_->setSolvedCoordinate({solve.raDeg,solve.decDeg,solve.frame},solve.success);}
    std::optional<EquatorialCoord> anchor;if(skyMap_->hasSelection())anchor=skyMap_->selectedCoordinate();else if(mountValid)anchor=st.coordinate;else if(solved.valid)anchor=solved.center;
    SkyFrame mainFrame,guideFrame;if(anchor){mainFrame=profileSkyFrame("main-profile","Main planned",*anchor,p.arcsecPerPixel(),p.sensorWidthPx,p.sensorHeightPx,skyMainRotation_?skyMainRotation_->value():0.0);guideFrame=profileSkyFrame("guide-profile","Guide planned",*anchor,p.guideArcsecPerPixel(),p.guideSensorWidthPx,p.guideSensorHeightPx,skyGuideRotation_?skyGuideRotation_->value():0.0);}skyMap_->setMainPlannedFrame(mainFrame);skyMap_->setGuidePlannedFrame(guideFrame);
    const int cols=mosaicCols_?mosaicCols_->value():3,rows=mosaicRows_?mosaicRows_->value():2;const double overlap=mosaicOverlap_?mosaicOverlap_->value():15.0;skyMap_->setPlannerMosaic(mainFrame,cols,rows,overlap,skyPlannerGrid_&&skyPlannerGrid_->isChecked());
    if(skyFrameInfo_){QStringList info;if(solved.valid)info<<QString("Solved %1° × %2° PA %3° (%4 px)").arg(solved.widthDeg,0,'f',3).arg(solved.heightDeg,0,'f',3).arg(solved.rotationDeg,0,'f',1).arg(QString("%1x%2").arg(solved.widthPx).arg(solved.heightPx));if(mainFrame.valid)info<<QString("Main profile %1° × %2° PA %3°").arg(mainFrame.widthDeg,0,'f',3).arg(mainFrame.heightDeg,0,'f',3).arg(mainFrame.rotationDeg,0,'f',1);if(guideFrame.valid)info<<QString("Guide %1° × %2° PA %3°").arg(guideFrame.widthDeg,0,'f',3).arg(guideFrame.heightDeg,0,'f',3).arg(guideFrame.rotationDeg,0,'f',1);if(skyPlannerGrid_&&skyPlannerGrid_->isChecked()&&mainFrame.valid)info<<QString("Mosaic %1×%2, overlap %3%").arg(cols).arg(rows).arg(overlap,0,'f',1);skyFrameInfo_->setText(info.isEmpty()?"Frames: no valid optical/solve context":info.join("   |   "));}
    updateSkySelectionText();
}

void MainWindow::updateSkyMapFromState(const QJsonObject&state){
    if(!skyMap_)return;const auto m=state.value("mount").toObject();if(m.isEmpty()){skyMap_->setMountCoordinate({},false);return;}const bool valid=!m.contains("coordinateValid")||m.value("coordinateValid").toBool();const auto frame=equatorialFrameFromString(m.value("coordinateFrame").toString("J2000"));skyMap_->setMountCoordinate({m.value("raDeg").toDouble(),m.value("decDeg").toDouble(),frame},valid);updateSkySelectionText();
}

void MainWindow::updateSkySelectionText(){
    if(!skySelection_||!skyMap_)return;if(!skyMap_->hasSelection()){skySelection_->setText("Select a catalogue object or click any point inside the horizon circle. Double-clicking either target sends a GOTO through the active OAL mount backend.");return;}const auto eq=skyMap_->selectedCoordinate();const auto h=skyMap_->selectedHorizontal();const double raHours=eq.raDeg/15.0;const bool custom=skyMap_->selectedKind()=="Point";const QString magnitude=custom?QString{}:QString("   mag %1").arg(skyMap_->selectedMagnitude(),0,'f',2);skySelection_->setText(QString("%1  [%2]   RA %3 h / %4°   DEC %5°   Alt %6°   Az %7°%8%9").arg(skyMap_->selectedName(),skyMap_->selectedKind()).arg(raHours,0,'f',4).arg(eq.raDeg,0,'f',5).arg(eq.decDeg,0,'f',5).arg(h.altDeg,0,'f',2).arg(h.azDeg,0,'f',2).arg(magnitude).arg(h.altDeg<0.0?"   — below horizon":""));
}
...
QWidget *MainWindow::buildDevicesTab(){auto*w=new QWidget;auto*l=new QVBoxLayout(w);
    ...
    for(const auto &entry:aux){auto*status=new QLabel(QString("Stub only — OAL type '%1' reserved; no backend/controls yet").arg(QString::fromLatin1(entry.second)));status->setWordWrap(true);status->setObjectName("oasMutedLabel");auxForm->addRow(entry.first,status);}
    l->addWidget(auxBox);
...
QWidget *MainWindow::buildLiveFinderTab(){
    ...
    liveExposure_=dspin(0.00001,10.0,0.001,5);liveGain_=ispin(0,102400,0);liveOffset_=ispin(0,65535,0);liveBin_=ispin(1,4,2);liveBits_=new QComboBox;liveBits_->addItems({"8-bit (high-rate)","16-bit"});liveCaptureFps_=dspin(0.0,1000.0,0.0,1);liveCaptureFps_->setSpecialValueText("Camera maximum");livePreviewFps_=dspin(0.0,240.0,60.0,1);livePreviewFps_->setSpecialValueText("As fast as possible");livePreviewWidth_=ispin(0,8192,1280);livePreviewWidth_->setSpecialValueText("Full frame");
    ...
    liveDebayer_=new QCheckBox("Debayer color preview (preview only; science data stays RAW)");liveDebayer_->setChecked(false);liveRedMonoPreview_=new QCheckBox("Black→red palette for monochrome/raw preview (display only; Debayer must be off)");{QSettings settings("OpenAstroLink","OpenAstroSuiteGui");liveRedMonoPreview_->setChecked(settings.value("ui/redMonoPreview",false).toBool());}liveBayerPattern_=new QComboBox;liveBayerPattern_->addItems({"Auto from camera/driver","RGGB","BGGR","GRBG","GBRG"});
    ...
    f->addRow("Exposure, s",liveExposure_);f->addRow("Gain",liveGain_);f->addRow("Offset",liveOffset_);f->addRow("Bin",liveBin_);f->addRow("Live bit depth",liveBits_);f->addRow("Capture FPS limit (0=max)",liveCaptureFps_);f->addRow("Preview FPS limit (0=max)",livePreviewFps_);f->addRow("Preview max width (0=full)",livePreviewWidth_);f->addRow(liveAutoStretch_);f->addRow(liveCrosshair_);f->addRow(liveMilDot_);f->addRow(liveAngularGrid_);f->addRow(liveHighlight_);f->addRow(liveDebayer_);f->addRow(liveRedMonoPreview_);f->addRow("Bayer pattern",liveBayerPattern_);f->addRow(liveRecordSer_);f->addRow("SER path on node",liveSerPath_);
    liveViewButton_=new QPushButton("Start Live View");sceneAutofocusButton_=new QPushButton("Scene autofocus (fast tile-contrast search)");liveTargetStatus_=new QLabel("Bright target: waiting for a frame");liveTargetStatus_->setWordWrap(true);f->addRow(liveViewButton_);f->addRow(sceneAutofocusButton_);f->addRow(liveTargetStatus_);l->addWidget(liveBox);
    auto*guideLiveBox=new QGroupBox("Guide camera Live View / optical-axis alignment");auto*gf=new QFormLayout(guideLiveBox);guideLiveExposure_=dspin(0.00001,10.0,0.01,5);guideLiveGain_=ispin(0,102400,0);guideLiveBin_=ispin(1,4,1);guideLiveBits_=new QComboBox;guideLiveBits_->addItems({"8-bit (high-rate)","16-bit"});guideLiveCaptureFps_=dspin(0.0,1000.0,0.0,1);guideLiveCaptureFps_->setSpecialValueText("Camera maximum");guideLivePreviewFps_=dspin(0.0,240.0,60.0,1);guideLivePreviewFps_->setSpecialValueText("As fast as possible");guideLivePreviewWidth_=ispin(0,8192,1280);guideLivePreviewWidth_->setSpecialValueText("Full frame");guideLiveViewButton_=new QPushButton("Start Guide Live");dualLiveButton_=new QPushButton("Start Main + Guide Live");gf->addRow("Guide exposure, s",guideLiveExposure_);gf->addRow("Guide gain",guideLiveGain_);gf->addRow("Guide bin",guideLiveBin_);gf->addRow("Live bit depth",guideLiveBits_);gf->addRow("Capture FPS limit (0=max)",guideLiveCaptureFps_);gf->addRow("Preview FPS limit (0=max)",guideLivePreviewFps_);gf->addRow("Preview max width (0=full)",guideLivePreviewWidth_);gf->addRow(guideLiveViewButton_);gf->addRow(dualLiveButton_);l->addWidget(guideLiveBox);
    connect(liveViewButton_,&QPushButton::clicked,this,[this](){
        ...
        LiveViewRequest r;r.exposureSec=liveExposure_->value();r.gain=liveGain_->value();r.offset=liveOffset_?liveOffset_->value():0;r.binX=r.binY=liveBin_->value();r.targetFps=0.0;r.captureFpsLimit=liveCaptureFps_->value();r.previewFpsLimit=livePreviewFps_->value();r.bitsPerSample=liveBits_&&liveBits_->currentIndex()>0?16:8;r.previewMaxWidth=livePreviewWidth_?livePreviewWidth_->value():1280;r.debayer=liveDebayer_&&liveDebayer_->isChecked();if(liveBayerPattern_){const QString p=liveBayerPattern_->currentIndex()==0?"AUTO":liveBayerPattern_->currentText();r.bayerPattern=bayerPatternFromString(p);}r.recordSer=liveRecordSer_&&liveRecordSer_->isChecked();if(liveSerPath_)r.serPath=liveSerPath_->text().trimmed();QString e;const QString id=c_->startLiveView(r,&e);if(id.isEmpty()){showError(e);return;}liveViewOperationId_=id;setLiveViewBusy(true);appendLog(QString("Live View started: %1").arg(id));
    });
    connect(guideLiveViewButton_,&QPushButton::clicked,this,[this](){
        if(guideLiveBusy_&&!guideLiveOperationId_.isEmpty()){QString e;if(!c_->cancelOperation(guideLiveOperationId_,&e))showError(e);else appendLog("Guide Live stop requested");return;}
        LiveViewRequest r;r.exposureSec=guideLiveExposure_->value();r.gain=guideLiveGain_->value();r.binX=r.binY=guideLiveBin_->value();r.captureFpsLimit=guideLiveCaptureFps_->value();r.previewFpsLimit=guideLivePreviewFps_->value();r.bitsPerSample=guideLiveBits_&&guideLiveBits_->currentIndex()>0?16:8;r.previewMaxWidth=guideLivePreviewWidth_?guideLivePreviewWidth_->value():1280;QString e;const QString id=c_->startGuideLiveView(r,&e);if(id.isEmpty()){showError(e);return;}guideLiveOperationId_=id;guideLiveBusy_=true;guideLiveViewButton_->setText("Stop Guide Live");appendLog("Guide Live started: "+id);
    });
    connect(dualLiveButton_,&QPushButton::clicked,this,[this](){
        if(liveViewBusy_||guideLiveBusy_){QString e;if(!liveViewOperationId_.isEmpty())c_->cancelOperation(liveViewOperationId_,&e);if(!guideLiveOperationId_.isEmpty())c_->cancelOperation(guideLiveOperationId_,&e);appendLog("Dual Live stop requested");return;}
        LiveViewRequest main;main.exposureSec=liveExposure_->value();main.gain=liveGain_->value();main.offset=liveOffset_->value();main.binX=main.binY=liveBin_->value();main.captureFpsLimit=liveCaptureFps_->value();main.previewFpsLimit=livePreviewFps_->value();main.bitsPerSample=liveBits_&&liveBits_->currentIndex()>0?16:8;main.previewMaxWidth=livePreviewWidth_?livePreviewWidth_->value():1280;main.debayer=liveDebayer_->isChecked();if(liveBayerPattern_){const QString p=liveBayerPattern_->currentIndex()==0?"AUTO":liveBayerPattern_->currentText();main.bayerPattern=bayerPatternFromString(p);}LiveViewRequest guide;guide.exposureSec=guideLiveExposure_->value();guide.gain=guideLiveGain_->value();guide.binX=guide.binY=guideLiveBin_->value();guide.captureFpsLimit=guideLiveCaptureFps_->value();guide.previewFpsLimit=guideLivePreviewFps_->value();guide.bitsPerSample=guideLiveBits_&&guideLiveBits_->currentIndex()>0?16:8;guide.previewMaxWidth=guideLivePreviewWidth_?guideLivePreviewWidth_->value():1280;QString e;const QString mid=c_->startLiveView(main,&e);if(mid.isEmpty()){showError(e);return;}liveViewOperationId_=mid;setLiveViewBusy(true);const QString gid=c_->startGuideLiveView(guide,&e);if(gid.isEmpty()){c_->cancelOperation(mid,nullptr);showError("Guide Live could not start: "+e);return;}guideLiveOperationId_=gid;guideLiveBusy_=true;guideLiveViewButton_->setText("Stop Guide Live");dualLiveButton_->setText("Stop Main + Guide Live");if(leftTabs_)leftTabs_->setCurrentIndex(1);appendLog(QString("Dual Live started: main=%1 guide=%2").arg(mid,gid));
    });
    ...
    connect(liveDebayer_,&QCheckBox::toggled,this,[this](bool enabled){if(enabled&&liveBin_&&liveBin_->value()!=1){liveBin_->setValue(1);appendLog("Live debayer enabled: bin set to 1x1 to preserve CFA color samples");}if(liveBayerPattern_)liveBayerPattern_->setEnabled(enabled);if(!lastImage_.isNull())renderCameraFrame(lastImage_);});liveBayerPattern_->setEnabled(false);connect(liveRedMonoPreview_,&QCheckBox::toggled,this,[this](bool enabled){QSettings settings("OpenAstroLink","OpenAstroSuiteGui");settings.setValue("ui/redMonoPreview",enabled);if(!lastImage_.isNull())renderCameraFrame(lastImage_);});
    ...
    auto*note=new QLabel("Safety: never point an unfiltered telescope/camera at the Sun. For daylight alignment use a distant terrestrial target. Daylight QHY start: about 1 ms, gain 0; if the image is white, reduce exposure before changing focus. QHY and ZWO Live View use native continuous SDK streams where available. Capture FPS and preview FPS are independent: 0 means maximum. Native Live defaults to 8-bit high-rate readout; 16-bit remains selectable. Remote preview is a latest-only binary OALV stream, can be downscaled independently, and never throttles raw SER recording. Debayer is optional and camera-neutral: Auto uses driver CFA metadata, while RGGB/BGGR/GRBG/GBRG can be selected manually. Science FITS/RAW and SER recording remain raw/undebayered. The optional black→red palette is display-only and applies only to monochrome/raw non-debayered previews; true color preview remains color. SER is uncompressed and can become very large at full resolution/FPS. Canon DSLR repeated-shutter Live View is intentionally blocked until the dedicated EDSDK EVF transport is added.");note->setWordWrap(true);l->addWidget(note);l->addStretch();return w;
}
...
void MainWindow::renderCameraFrame(const QImage &image){
    ...
    if(liveMilDot_&&liveMilDot_->isChecked()&&scaleArcsecPerPx>0.0){
        ...
        if(stepPx>=4.0&&stepPx<std::max(shown.width(),shown.height())*2.0){
            p.setPen(QPen(themeColor(QColor(255,190,80,180)),std::max(1,shown.width()/1600),Qt::DotLine));
            ...
            p.setPen(themeColor(QColor(255,210,100)));p.drawText(QPointF(8,18),QString("Mil-Dot: 1 mrad = %1 px").arg(stepPx,0,'f',1));
        }
    }
    if(liveAngularGrid_&&liveAngularGrid_->isChecked()&&scaleArcsecPerPx>0.0){
        ...
        p.setPen(QPen(themeColor(QColor(100,190,255,150)),std::max(1,shown.width()/1800),Qt::DashLine));
        for(double x=center.x();x<shown.width();x+=stepPx)p.drawLine(QPointF(x,0),QPointF(x,shown.height()));
        for(double x=center.x()-stepPx;x>=0;x-=stepPx)p.drawLine(QPointF(x,0),QPointF(x,shown.height()));
        for(double y=center.y();y<shown.height();y+=stepPx)p.drawLine(QPointF(0,y),QPointF(shown.width(),y));
        for(double y=center.y()-stepPx;y>=0;y-=stepPx)p.drawLine(QPointF(0,y),QPointF(shown.width(),y));
        p.setPen(themeColor(QColor(140,210,255)));p.drawText(QPointF(8,36),QString("Angular grid: %1′  scale=%2″/px").arg(stepArcmin,0,'g',3).arg(scaleArcsecPerPx,0,'f',3));
    }
    bool valid=false;double contrast=0.0;const QPointF bright=brightestRegion(image,&valid,&contrast);
    if(liveHighlight_&&liveHighlight_->isChecked()&&valid){p.setPen(QPen(themeColor(QColor(255,210,40)),2));const int r=std::max(14,std::min(shown.width(),shown.height())/40);p.drawEllipse(bright,r,r);p.drawLine(bright+QPointF(-r*1.4,0),bright+QPointF(r*1.4,0));p.drawLine(bright+QPointF(0,-r*1.4),bright+QPointF(0,r*1.4));}
...
void MainWindow::updateHistogram(const QImage &image,bool allowAutoApply){
    ...
    if(!total)return;
    auto percentile=[&](double p){const quint64 targetCount=quint64(std::clamp(p,0.0,1.0)*double(total-1));quint64 acc=0;for(int i=0;i<256;++i){acc+=bins[size_t(i)];if(acc>targetCount)return i;}return 255;};
    const int p01=percentile(0.01),p50=percentile(0.50),p95=percentile(0.95),p99=percentile(0.99),p995=percentile(0.995);
    const double lowClip=double(bins[0]+bins[1])/double(total),highClip=double(bins[254]+bins[255])/double(total);
    const int W=560,H=170;QImage plot(W,H,QImage::Format_ARGB32_Premultiplied);plot.fill(uiTheme_==UiTheme::StrictNight?QColor(3,0,0):QColor(16,16,16));QPainter painter(&plot);painter.setRenderHint(QPainter::Antialiasing,false);
    double maxLog=1.0;for(auto n:bins)maxLog=std::max(maxLog,std::log1p(double(n)));
    painter.setPen(QPen(themeColor(QColor(170,190,220)),1));for(int i=0;i<256;++i){const double h=std::log1p(double(bins[size_t(i)]))/maxLog;const int x=int(std::lround(double(i)*(W-1)/255.0));painter.drawLine(x,H-2,x,H-2-int(std::lround(h*(H-12))));}
    const double backgroundTarget=histogramTarget_?histogramTarget_->value()/100.0:0.18;
    const int targetX=int(std::lround(std::clamp(backgroundTarget,0.0,1.0)*(W-1)));painter.setPen(QPen(themeColor(QColor(90,220,120)),1,Qt::DashLine));painter.drawLine(targetX,0,targetX,H);
    painter.setPen(QPen(themeColor(QColor(240,190,80)),1));const int medX=int(std::lround(double(p50)*(W-1)/255.0));painter.drawLine(medX,0,medX,H);painter.end();histogramView_->setPixmap(QPixmap::fromImage(plot));

    const double median=double(p50)/255.0,p95Level=double(p95)/255.0,p99Level=std::max(1.0/255.0,double(p99)/255.0),p995Level=std::max(1.0/255.0,double(p995)/255.0);
    if(!allowAutoApply){
        histogramStats_->setText(QString("Sensor-scale preview only: P1=%1%, median=%2%, P95=%3%, P99.5=%4%, clip=%5%. Live/AF frames do not change the still-capture controller.")
            .arg(100.0*p01/255.0,0,'f',1).arg(100.0*median,0,'f',1).arg(100.0*p95Level,0,'f',1).arg(100.0*p995Level,0,'f',1).arg(100.0*highClip,0,'f',2));
        histogramApplyButton_->setEnabled(false);return;
    }

    // A global median is useful for a filled deep-sky/background field, but it
    // is the wrong control signal for a small Moon/planet/daylight feature on a
    // mostly dark frame. That exact case caused the HIL controller to increase
    // exposure until the target clipped, then bounce between two exposures.
    // Detect a sparse bright-tail scene and control P99.5 instead, while keeping
    // the user's background target for ordinary filled fields.
    const bool sparseBright=(p995Level>0.20&&p995Level>std::max(0.12,median*3.8)&&p995Level>p95Level*1.7);
    const double controlTarget=sparseBright?0.62:std::clamp(backgroundTarget,0.03,0.40);
    const double controlMeasured=std::max(1.0/255.0,sparseBright?p995Level:median);
    const double current=std::max(0.0001,exposure_?exposure_->value():1.0);
    const int currentGain=gain_?gain_->value():0;
    if(histogramAutoLastGain_>=0&&currentGain!=histogramAutoLastGain_){histogramAutoBestExposure_=0.0;histogramAutoBestObjective_=1.0e9;histogramAutoHavePrevious_=false;histogramAutoConverged_=false;histogramAutoOutOfBandFrames_=0;}
    histogramAutoLastGain_=currentGain;

    const double signedError=std::log(controlMeasured/controlTarget);
    const double relativeError=std::abs(controlMeasured-controlTarget)/std::max(0.01,controlTarget);
    const double objective=std::abs(signedError)+12.0*highClip+std::max(0.0,p995Level-0.88)*4.0;
    if(histogramAutoBestExposure_<=0.0||objective<histogramAutoBestObjective_){histogramAutoBestObjective_=objective;histogramAutoBestExposure_=current;}

    const bool healthyHighlights=highClip<=0.0050&&p995Level<=0.96;
    const bool inBand=relativeError<=0.12&&healthyHighlights;
    ...
    if(inBand){histogramAutoConverged_=true;histogramAutoBestExposure_=current;histogramAutoBestObjective_=objective;histogramAutoOutOfBandFrames_=0;}
    if(histogramAutoConverged_){
        const bool sceneChanged=relativeError>0.45||highClip>0.02||p995Level>0.985;
    ...
    double next=current;
    if(!histogramAutoConverged_){
        double factor=std::pow(std::clamp(controlTarget/controlMeasured,0.10,10.0),0.45);
        factor=std::clamp(factor,0.65,1.60);
        // Smooth highlight protection: never use the old hard 0.5% clipping
        // threshold that made the HIL sequence alternate ~0.64s <-> ~1.43s.
        if(p995Level>0.88)factor=std::min(factor,std::clamp(std::pow(0.88/p995Level,0.55),0.68,0.99));
        if(highClip>0.005)factor=std::min(factor,std::clamp(std::pow(0.005/highClip,0.35),0.60,0.94));
        if(lowClip>0.90&&p995Level<0.12)factor=std::max(factor,1.20);
        next=std::clamp(current*factor,0.0001,3600.0);
        // When the controlled level crosses its target, bisect in log/exposure
        // space rather than overshooting back across the same boundary.
        if(crossed&&histogramAutoPreviousExposure_>0.0){
            next=std::sqrt(current*histogramAutoPreviousExposure_);
            if(std::max(current,histogramAutoPreviousExposure_)/std::max(0.0001,std::min(current,histogramAutoPreviousExposure_))<1.18&&healthyHighlights){histogramAutoConverged_=true;next=histogramAutoBestExposure_>0.0?histogramAutoBestExposure_:next;}
        }
        if(std::abs(next/current-1.0)<0.035&&healthyHighlights){histogramAutoConverged_=true;next=histogramAutoBestExposure_>0.0?histogramAutoBestExposure_:current;}
    }else if(histogramAutoBestExposure_>0.0)next=histogramAutoBestExposure_;

    histogramAutoPreviousSignedError_=signedError;histogramAutoPreviousExposure_=current;histogramAutoHavePrevious_=true;
    histogramSuggestedExposure_=next;const double delta=std::abs(histogramSuggestedExposure_/current-1.0);
    const QString mode=sparseBright?"bright-tail P99.5":"background median";
    QString advice;
    if(histogramAutoConverged_)advice=QString("AUTO LOCKED near optimum (%1 s)").arg(histogramAutoBestExposure_,0,'g',5);
    else if(highClip>0.0050)advice="highlight clipping detected — shortening smoothly";
    else if(controlMeasured<controlTarget*0.88)advice="controlled level low — increase exposure";
    else if(controlMeasured>controlTarget*1.12)advice="controlled level high — decrease exposure";
    else advice="inside acquisition band — locking exposure";
    histogramStats_->setText(QString("Sensor-scale histogram [%1]: median=%2%, P95=%3%, P99=%4%, P99.5=%5%, clip=%6%. %7. Best=%8 s; suggested=%9 s.")
        .arg(mode).arg(100.0*median,0,'f',1).arg(100.0*p95Level,0,'f',1).arg(100.0*p99Level,0,'f',1).arg(100.0*p995Level,0,'f',1).arg(100.0*highClip,0,'f',2).arg(advice).arg(histogramAutoBestExposure_,0,'g',5).arg(histogramSuggestedExposure_,0,'g',5));
    histogramApplyButton_->setEnabled(!histogramAutoConverged_&&delta>0.025);
    if(allowAutoApply&&histogramAutoExposure_&&histogramAutoExposure_->isChecked()&&!histogramAutoConverged_&&delta>0.025){exposure_->setValue(histogramSuggestedExposure_);appendLog(QString("Histogram auto exposure: next capture %1 s (%2 controller, gain=%3)").arg(histogramSuggestedExposure_,0,'g',5).arg(mode).arg(currentGain));}
    else if(allowAutoApply&&histogramAutoExposure_&&histogramAutoExposure_->isChecked()&&histogramAutoConverged_){appendLog(QString("Histogram auto exposure LOCKED at %1 s (%2); median=%3%, P99.5=%4%, clip=%5%").arg(histogramAutoBestExposure_,0,'g',5).arg(mode).arg(100.0*median,0,'f',1).arg(100.0*p995Level,0,'f',1).arg(100.0*highClip,0,'f',2));}
}
...
QWidget *MainWindow::buildMountTab(){
    ...
    auto*orientationBox=new QGroupBox("Native raw-axis orientation calibration");
    auto*orientationLayout=new QVBoxLayout(orientationBox);
    auto*orientationInfo=new QLabel("These signs apply ONLY to native raw-axis EQDrive/SynScan geometry. Classic ASCOM owns its own motor/sky mapping. The checkboxes show the CURRENT mapping state; changing them alters only the profile sign, not the HIL-qualified v9 geometry equations.");
    orientationInfo->setWordWrap(true);orientationLayout->addWidget(orientationInfo);
    mountInvertAxis1_=new QCheckBox("Invert Axis 1 mapping / tracking direction");
    mountInvertAxis2_=new QCheckBox("Invert Axis 2 mapping");
    {const auto p=c_->profile();mountInvertAxis1_->setChecked(p.mount.axis1Sign<0);mountInvertAxis2_->setChecked(p.mount.axis2Sign<0);}
    auto nativeOnly=[this](){MountStatus st;QString e;if(c_->mountStatus(st,&e)&&st.geometryType=="ascom-classic"){showError("Axis sign mapping applies only to native raw-axis mounts. ASCOM/EQMOD owns its own axis mapping; fix site/time or the ASCOM driver configuration instead.");return false;}return true;};
    connect(mountInvertAxis1_,&QCheckBox::toggled,this,[this,nativeOnly](bool inverted){if(!nativeOnly()){QSignalBlocker b(mountInvertAxis1_);mountInvertAxis1_->setChecked(c_->profile().mount.axis1Sign<0);return;}auto p=c_->profile();const int sign=inverted?-1:1;if(p.mount.axis1Sign==sign)return;p.mount.axis1Sign=sign;c_->setProfile(p);appendLog(QString("Axis 1 mapping inversion %1 (sign=%2); native sky model will be restored from Home or refined by Sync").arg(inverted?"ON":"OFF").arg(sign));});
    connect(mountInvertAxis2_,&QCheckBox::toggled,this,[this,nativeOnly](bool inverted){if(!nativeOnly()){QSignalBlocker b(mountInvertAxis2_);mountInvertAxis2_->setChecked(c_->profile().mount.axis2Sign<0);return;}auto p=c_->profile();const int sign=inverted?-1:1;if(p.mount.axis2Sign==sign)return;p.mount.axis2Sign=sign;c_->setProfile(p);appendLog(QString("Axis 2 mapping inversion %1 (sign=%2); native sky model will be restored from Home or refined by Sync").arg(inverted?"ON":"OFF").arg(sign));});
    orientationLayout->addWidget(mountInvertAxis1_);orientationLayout->addWidget(mountInvertAxis2_);l->addWidget(orientationBox);
...
QWidget *MainWindow::buildSchedulerTab(){
    ...
    auto refreshPlanner=[this](){refreshSkyMapContext();};connect(mosaicCols_,qOverload<int>(&QSpinBox::valueChanged),this,[refreshPlanner](int){refreshPlanner();});connect(mosaicRows_,qOverload<int>(&QSpinBox::valueChanged),this,[refreshPlanner](int){refreshPlanner();});connect(mosaicOverlap_,qOverload<double>(&QDoubleSpinBox::valueChanged),this,[refreshPlanner](double){refreshPlanner();});connect(mosaicRotation_,qOverload<double>(&QDoubleSpinBox::valueChanged),this,[this](double v){if(skyMainRotation_&&std::abs(skyMainRotation_->value()-v)>1e-9){QSignalBlocker b(skyMainRotation_);skyMainRotation_->setValue(v);}refreshSkyMapContext();});
...
QWidget *MainWindow::buildServerTab(){auto*w=new QWidget;auto*l=new QVBoxLayout(w);
    ...
    auto*note=new QLabel("Stellarium's standard Telescope Control protocol provides mount position and GOTO. Sky Map can additionally export one selected OAL camera/planner footprint at a time through Stellarium's optional Remote Control HTTP plugin (default port 8090). Camera, focuser, autofocus, polar alignment and sessions remain controlled through OpenAstroLink.");note->setWordWrap(true);l->addWidget(note);l->addStretch();return w;}
...
void MainWindow::setLiveViewBusy(bool busy){liveViewBusy_=busy;if(liveViewButton_)liveViewButton_->setText(busy?"Stop Live View":"Start Live View");if(dualLiveButton_)dualLiveButton_->setText((busy||guideLiveBusy_)?"Stop Main + Guide Live":"Start Main + Guide Live");if(sceneAutofocusButton_)sceneAutofocusButton_->setEnabled(!busy&&!autofocusBusy_);if(captureButton_)captureButton_->setEnabled(!busy);if(captureSolveButton_)captureSolveButton_->setEnabled(!busy);if(adaptiveSolveButton_)adaptiveSolveButton_->setEnabled(!busy);if(busy)statusBar()->showMessage("Main Live View running — high-rate capture is decoupled from droppable preview; mount/focuser controls remain available");else statusBar()->showMessage(QString("Core: %1 — %2").arg(c_->controlMode(),c_->endpointDescription()));}
...
void MainWindow::updateDeviceStatusFromState(const QJsonObject&state){
    ...
    for(const auto &x:ui){if(!x.status)continue;QJsonObject found;for(const auto&v:devices){auto o=v.toObject();const QString role=o.value("role").toString("main");if(o.value("type").toString()==x.type&&role==x.role&&o.value("connected").toBool()){found=o;break;}}if(found.isEmpty()){x.status->setText("Disconnected");x.status->setStyleSheet(QString("color:%1").arg(themeColor(QColor(170,0,0)).name()));continue;}const QString backend=found.value("backend").toString();const QString endpoint=found.value("endpoint").toString();const QString name=found.value("name").toString();x.status->setText(QString("Connected — %1 (%2)").arg(name,backend));x.status->setStyleSheet(QString("color:%1").arg(themeColor(QColor(0,136,0)).name()));if(x.backend){int i=x.backend->findText(backend);if(i>=0)x.backend->setCurrentIndex(i);}if(x.endpoint&&!endpoint.isNull())x.endpoint->setText(endpoint);}
}
void MainWindow::updateOperation(const QJsonObject&o){if(o.isEmpty())return;const QString id=o.value("id").toString();if(id.isEmpty())return;const QString state=o.value("state").toString();const QString kind=o.value("kind").toString();const int pct=int(o.value("progress").toDouble()*100.0);QStringList locks;for(const auto&v:o.value("resourceLocks").toArray())locks<<v.toString();const QString text=QString("%1  %2  %3  %4%  [%5]").arg(kind,id,state).arg(pct).arg(locks.join(","));if(operationsList_){QListWidgetItem*item=nullptr;for(int i=0;i<operationsList_->count();++i)if(operationsList_->item(i)->data(Qt::UserRole).toString()==id){item=operationsList_->item(i);break;}if(!item){item=new QListWidgetItem(operationsList_);item->setData(Qt::UserRole,id);}item->setText(text);}
    ...
    if(kind=="camera.guide.live-view"&&(state=="queued"||state=="running")&&guideLiveOperationId_.isEmpty()){guideLiveOperationId_=id;guideLiveBusy_=true;if(guideLiveViewButton_)guideLiveViewButton_->setText("Stop Guide Live");if(dualLiveButton_)dualLiveButton_->setText("Stop Main + Guide Live");}
    if(id==guideLiveOperationId_&&(state=="succeeded"||state=="failed"||state=="cancelled")){if(state=="failed")appendLog("Guide Live failed: "+o.value("problem").toObject().value("message").toString());else appendLog("Guide Live stopped");guideLiveBusy_=false;guideLiveOperationId_.clear();if(guideLiveViewButton_)guideLiveViewButton_->setText("Start Guide Live");if(dualLiveButton_)dualLiveButton_->setText(liveViewBusy_?"Stop Main + Guide Live":"Start Main + Guide Live");}
...
QColor MainWindow::themeColor(const QColor &dayColor) const { return isStrictNight(uiTheme_)?strictNightMappedColor(dayColor):dayColor; }
QImage MainWindow::displayPreviewImage(const QImage &image,bool debayered) const {
    if(image.isNull())return image;
    if(liveRedMonoPreview_&&liveRedMonoPreview_->isChecked()&&!debayered&&monochromeLike(image))return blackToRedPreview(image);
    return image.convertToFormat(QImage::Format_RGB888);
}
void MainWindow::refreshThemeDependentViews(){
    const bool night=uiTheme_!=UiTheme::Normal;
    const QString previewStyle=night?QStringLiteral("background:#020000;color:#9e2e2e;border:1px solid #3c0b0b"):QStringLiteral("background:#111;color:#bbb;border:1px solid #444");
    for(auto*l:{rawImage_,astroImage_,dualMainImage_,dualGuideImage_})if(l)l->setStyleSheet(previewStyle);
    if(starMapView_)starMapView_->setStyleSheet(night?QStringLiteral("background:#020000;border:1px solid #3c0b0b"):QStringLiteral("background:#050510"));
    if(histogramView_)histogramView_->setStyleSheet(night?QStringLiteral("background:#020000;color:#9e2e2e;border:1px solid #3c0b0b"):QStringLiteral("background:#101010;color:#bbb;border:1px solid #444"));
    for(auto*l:findChildren<QLabel*>(QStringLiteral("oasWarningLabel")))l->setStyleSheet(QString("color:%1;font-weight:bold").arg(themeColor(QColor(170,68,0)).name()));
    for(auto*l:findChildren<QLabel*>(QStringLiteral("oasMutedLabel")))l->setStyleSheet(QString("color:%1").arg(themeColor(QColor(119,119,119)).name()));
    if(skyMap_)skyMap_->setNightVisionMode(night,isStrictNight(uiTheme_));
    if(!lastImage_.isNull())renderCameraFrame(lastImage_);
    if(histogramEnabled_&&histogramEnabled_->isChecked()&&!lastImage_.isNull())updateHistogram(lastImage_,false);
    if(c_){updateAstrometryOverlay();updateStarMap();}
}
void MainWindow::applyTheme(UiTheme theme,bool persist){
    uiTheme_=theme;if(qApp)oas::applyUiTheme(*qApp,theme);if(persist)saveUiTheme(theme);refreshThemeDependentViews();statusBar()->showMessage(QString("%1 — Core: %2 — %3").arg(uiThemeName(theme),c_->controlMode(),c_->endpointDescription()),5000);
}
void MainWindow::appendLog(const QString&s){log_->append(QTime::currentTime().toString("HH:mm:ss ")+s);}void MainWindow::showError(const QString&s){appendLog("ERROR: "+s);QMessageBox::warning(this,"OpenAstroSuite",s);}void MainWindow::updateAstrometryOverlay(){if(lastImage_.isNull())return;QImage o=lastImage_.copy();QPainter p(&o);p.setPen(QPen(themeColor(QColor(Qt::green)),2));for(const auto&s:c_->lastSolve().imageStars)p.drawEllipse(QPointF(s.positionPx.x,s.positionPx.y),6,6);p.setPen(themeColor(QColor(Qt::yellow)));p.drawText(12,24,QString("RA %1  DEC %2  PA %3  matches %4").arg(c_->lastSolve().raDeg,0,'f',5).arg(c_->lastSolve().decDeg,0,'f',5).arg(c_->lastSolve().rotationDeg,0,'f',2).arg(c_->lastSolve().matchedStars));astroImage_->setPixmap(QPixmap::fromImage(o).scaled(astroImage_->size(),Qt::KeepAspectRatio,Qt::SmoothTransformation));}void MainWindow::updateStarMap(){starScene_->clear();starScene_->setSceneRect(0,0,600,200);starScene_->addRect(starScene_->sceneRect(),QPen(themeColor(QColor(Qt::darkBlue))),QBrush(uiTheme_==UiTheme::Normal?QColor(3,3,16):QColor(3,0,0)));if(c_->lastFrame().image.empty())return;double sx=600.0/c_->lastFrame().image.cols,sy=200.0/c_->lastFrame().image.rows;for(const auto&s:c_->lastSolve().imageStars){double r=std::clamp(2.0+std::log1p(s.flux),2.0,8.0);starScene_->addEllipse(s.positionPx.x*sx-r,s.positionPx.y*sy-r,2*r,2*r,QPen(themeColor(QColor(Qt::white))),QBrush(themeColor(QColor(Qt::white))));}auto*t=starScene_->addText(QString("Solved center RA %1° DEC %2°").arg(c_->lastSolve().raDeg,0,'f',4).arg(c_->lastSolve().decDeg,0,'f',4));t->setDefaultTextColor(themeColor(QColor(Qt::cyan)));t->setPos(8,8);}
} // namespace oas
