...
const char *manifest(void*){
    return copyString(R"({"driverId":"oal.simulated","name":"OpenAstroLink native reference simulator","version":"0.2.10.19","abiVersion":2,"threadModel":"per-device-serial","capabilityModel":"typed-json-v1"})");
}
...
OalDriverV2 api{OAL_DRIVER_ABI_V2,sizeof(OalDriverV2),OAL_DRIVER_FEATURE_EVENTS|OAL_DRIVER_FEATURE_FRAME_PUBLISH|OAL_DRIVER_FEATURE_CANCELLATION|OAL_DRIVER_FEATURE_HEALTH,
                "oal.simulated","OpenAstroLink native reference simulator","0.2.10.19",nullptr,&manifest,&start,&stop,&devices,&caps,&health,&invoke,&cancel,&releaseString};
} // namespace
